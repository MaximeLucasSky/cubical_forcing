...
(** Should be replaced by Implement **)
(*
  ...
  | Success tA' =>
    id' <- tmEval all (tsl_id id) ;;
        tmBind (tmUnquoteTyped Type tA') (fun A' =>
                                            tmPrint "On towards Lemma";;
                                                    tmPrint A' ;;
                                                    tmLemma id' A' ;;
                                                    tmPrint "Lemma Done, On Towards Axiom";;
                                                    tmAxiom id A ;;
                                                    gr <- tmAbout id ;;
      match gr with
      | Some (ConstRef kn) =>
...
Fixpoint get_global_consts (tm : term) : list ident :=
  ...
  | tProd _ t1 t2 => get_global_consts t1 ++ get_global_consts t2
  | tLambda _ t1 t2 => get_global_consts t1 ++ get_global_consts t2
  | tLetIn _ t1 t2 t3 => get_global_consts t1 ++ get_global_consts t2 ++  get_global_consts t3
